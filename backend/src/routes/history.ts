...
router.get('/:id/export', (req: Request, res: Response) => {
  ...
  if (format === 'txt') {
    ...
    res.setHeader('Content-Disposition', `attachment; filename="${encodeURIComponent(session.title || session.id)}.txt"`);
    return res.send(sessionToText(session));
  }
  res.setHeader('Content-Type', 'application/json');
  res.setHeader('Content-Disposition', `attachment; filename="${encodeURIComponent(session.title || session.id)}.json"`);
